...
public class PedidoService {
    ...
    @Autowired
    private CodigoDescontoRepository codigoDescontoRepository; //LUCAS
	...
	public PedidoFinalizadoResponseDto finalizarPedido(String codigoDesconto) {
	    String email = SecurityContextHolder.getContext().getAuthentication().getName();
	    ...
	    for (ItemPedido item : pedido.getItens()) {
	        ...
	        // Atualiza preço unitário promocional se houver
	        if (produto.getPrecoPromocional() != null && produto.getPrecoPromocional().compareTo(BigDecimal.ZERO) > 0) {
	    ...
	    // Aqui aplica o código de desconto (LUCAS)
	    BigDecimal desconto = BigDecimal.ZERO;
	    if (codigoDesconto != null && !codigoDesconto.isEmpty()) {
	        Optional<CodigoDesconto> codOpt = codigoDescontoRepository.findByCodigo(codigoDesconto);
	        if (codOpt.isPresent() && codOpt.get().isAtivo()) {
	            desconto = codOpt.get().getValorDesconto();
	            pedido.setCodigoDesconto(codigoDesconto);
	        } else {
	            pedido.setCodigoDesconto(null);
	        }
	    } else {
	        pedido.setCodigoDesconto(null);
	    }
	    ...
	    BigDecimal totalItens = valorTotal(pedido);

	    // Aplica o desconto, garantindo que não seja negativo
	    BigDecimal valorFinal = totalItens.subtract(desconto);
	    if (valorFinal.compareTo(BigDecimal.ZERO) < 0) {
	        valorFinal = BigDecimal.ZERO;
	    }

	    pedido.setValorFinal(valorFinal);
	    pedido.setPrecoTotal(totalItens);
